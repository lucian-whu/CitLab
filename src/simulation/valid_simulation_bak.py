...
def simulated_data_viz(length):

    author_year_articles_sim = json.loads(open('data/simulate_author_year_papers_{:}.json'.format(length)).read())
    ...
    plt.tight_layout()
    plt.savefig('fig/simulated_pn.pdf',dpi=400)
    print 'simulation of total papers saved to fig/simulated_pn.pdf'
    ...
    plt.tight_layout()
    plt.savefig('fig/simulated_tn_dis.pdf',dpi=400)
    print 'data saved to fig/simulated_tn_dis.pdf'


    ### 6位最高产作者的可视化

    fig,axes= plt.subplots(2,3,figsize=(24,8))
    for i,tn in enumerate(sorted(tna_prod_dis.keys(),reverse=True)[:6]):
...
def validate_simulation(mode,length):
    ...
    plt.plot(xs[1:],ys[1:])
    plt.title(tit,fontproperties='SimHei')
    plt.xlabel(u'发表年份',fontproperties='SimHei')
    plt.ylabel(u'价值',fontproperties='SimHei')
    plt.tight_layout()
    plt.savefig(outpath+'simulated_year_average_kg_{:}_{:}.png'.format(mode,length),dpi=600)
    print 'fig saved tooutpath+ simulated_year_average_kg_{:}_{:}.png'.format(mode,length)
